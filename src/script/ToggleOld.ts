 ...
 * @class ToggleOld
 */
export default class ToggleOld extends Events {
  base: HTMLElement;
  body: HTMLElement;
  button: HTMLElement;
   ...
   */
  static baseName: string = 'toggleOld';

  /**
   * 現在の HTML ページ内にあるすべての Toggle ブロックをインスタンス化する
   */
  static createAll(name: string = ToggleOld.baseName) {
    [...document.getElementsByClassName(`${name}`)].forEach((element: Object) => {
      return new ToggleOld(element, name);
    });
  }
   ...
   */
  constructor(element: Object, name: string = ToggleOld.baseName) {
    super();
